...
""" ma+sar
trend long: ma20 > ma50, sar > close, SAR 金叉做多
trend short: ma50 < ma50, sar < close, SAR 死叉做空
...
class SARStrategyClass(BaseStrategyClass):
    '''sar
    '''
    ...
    def next(self):
        ...
        # 做多信号计算
        if (
            self.ma20[0] > self.ma50[0] and self.ma50[0] > self.ma150[0]
            and bias < 1
            and self.sar[0] > 0 and self.sar[-1] < 0
            and self.macd_diff[0] > self.macd_dea[0]
            and self.macd_diff[0] > self.macd_diff[-1]
            # and self.macd_diff[-20] < self.macd_dea[-20]
            # and any([self.macd_diff[i-30] < self.macd_dea[i-30] for i in range(30)])
